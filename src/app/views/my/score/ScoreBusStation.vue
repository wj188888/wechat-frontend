        ...
        <div class="buy-left clearfix" @click="showPriceDetail = !showPriceDetail">
          ...
          <span>
           {{ totalPrice }}<i class="small">积分</i>
            <em v-if="totalServicePrice"><i class="small"> + ¥</i>{{totalServicePrice}}</em>
          </span>
          <span class="detail" v-show="priceMore">详情</span>
          <i class="yellow-jt-up" v-show="priceMore"></i>
        </div>
        <div class="buy-right">
          <input type="button" @click="orderCreateHandel('station')" :disabled="disableBtn" class="button button-primary" value="提交订单"/>
        </div>
      </div>
    ...
    computed: {
      totalPrice () {
        ...
        if (this.passengerNum > 0) {
          this.priceMore = true
          let ticketPrice = this.ticketInfo.price * this.passengerNum
          ...
          this.totalServicePrice = insureprice + this.servicePrice
