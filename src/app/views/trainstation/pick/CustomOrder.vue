    ...
    computed: {
      totalPrice () {
        if(this.price > 0){
          ...
          return this.price + this.productOne.serviceFee - this.reducedAmount - this.orderCouponPrice - this.integralPrice
        }else{
          return this.price
        }
      ...
      getLocalData () {
        if (appStorage.get(this.pageRouter)) {
          ...
          if (this.orderData.offStation.name) {
            this.offAddress = this.productOne.aAddress.city + ' ' + this.orderData.offStation.name
          }
