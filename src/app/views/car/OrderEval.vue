<template>
  <main-box :title="title" :show-more="false" main-class="scrollingBox bg-ffffff">
  ...
  export default {
    ...
    data () {
      return {
        ...
        newevalText: [],
        evalText: [
          {text:'司机认路很准哦',isSelected: false},
          {text:'司机活地图',isSelected: false},
          {text:'车内干净',isSelected: false},
          {text:'服务态度好',isSelected: false}
        ],
        disableBtn: false,
    ...
    methods: {
      init() {
        ...
        this.webType = this.$route.query.webType;
        this.orderReviewFindOrderDetail();
        if(this.webType == 'watch') {
          this.getOrderDetail();
        }
      },
       ...
       * 查询司机信息
       */
      orderReviewFindOrderDetail () {
        api.orderReviewFindOrderDetail(this.orderId, this.productTypeLevelOne).then(res => {
