    ...
    <div class="ticket-scroller">
      <scroll ref="scroll"
        ...
        <div class="scroller-box">
          <special-ticket :title="SCENIC_SPECIAL_BUS" :skus="specialTicketSku" @jumpToPage="jumpToAppointBus" @showDiscountDialog="showDiscountDialog"></special-ticket>
          <express-ticket :title="SCENIC_EXPRESS_BUS" :skus="expressSkus" @jumpToPage="jumpToBusDetail" @showDiscountDialog="showDiscountDialog"></express-ticket>
  ...
  export default {
    ...
    data () {
      return {
        ...
        SCENIC_SPECIAL_BUS: TOOL.SCENIC_SPECIAL_BUS,
        SCENIC_EXPRESS_BUS: TOOL.SCENIC_EXPRESS_BUS,
        calendar: {
    ...
    methods: {
      initialize () {
        ...
        this.pageId = +this.$route.query.pageId || 28
      ...
      scenicProductFind () {
        return api.scenicProductFind(this.searchKeys).then(res=>{
          return Promise.resolve(res.data);
        })
      },
  
      scenicSkuFind () {
        return api.scenicSkuFind(this.searchKeys).then(res=>{
          return Promise.resolve(res.data);
        })
      },
      ...
      getAllProduct () {
        return Promise.all([
          api.scenicProductFind(this.searchKeys),
          api.scenicSkuFind(this.searchKeys)
        ]);
      },
      ...
      async loadPageList () {
        ...
        switch (this.pageId) {
          case 28: //全部
            ...
            break;
          case 2801: // 专车
            specialResult = await this.scenicProductFind()
            break;
          default: // 快车 2802
            this.searchKeys.type = 1
            esResult = await this.scenicSkuFind()
        }
        ...
        if(esResult.resultCode === '0'){
          this.expressSkus = esResultData.scenicSkus && this.expressSkus.concat(esResultData.scenicSkus.content)
          this.totalPage = esResultData.scenicSkus && esResultData.scenicSkus.totalPages
        }
    
        if (this.pageId === 28 && this.expressSkus.length === 0) {
          ...
          if((this.pageId === 2801 && !this.specialTicketSku) || (this.pageId === 2802 && this.expressSkus.length === 0)){
            this.showNoResult = true
            this.noResultText = '该时段的车都被预定完了耶，换个试试吧~'
          }else{
      ...
      deleteAppointBusForm () {
        appStorage.remove('scenicBusSpecial')
      },
       ...
       */
      deleteBusDetailForm () {
        appStorage.remove('scenicBusExpress')
