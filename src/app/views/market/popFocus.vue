<template>
  <main-box :title="title" :show-more="false" main-class="scrollingBox">
    <div class="pop">
      ...
      <p class="pop__p2" v-if="followRewardSetting.rewardType == '1'">1、可累计计算奖励。</p>
      <p class="pop__p2" v-else>1、阶梯计算奖励，到达提现日期，计算最高达到的奖励，不可累计计算奖励。</p>
      <p class="pop__p2" v-if="!!followRewardSetting.limit&&followRewardSetting.limitFlag==1">2、每{{rewardDate[followRewardSetting.limitUnit]}}总的奖励金额限制在{{followRewardSetting.limit}}元以内，先领先得。</p>
      ...
      <div class="pop__table-box" v-if="followRewardSetting.rewardType == '1'">
        <table>
          <tr>
            <th>推广关注</th>
            <th>奖励金额</th>
          </tr>
          <tr v-for="(val, index) of followRewardSetting.rewardDetailList">
            <td>{{val.above}}人</td>
      ...
      <div v-else>
        <p class="pop__p2" v-for="(val, index) of followRewardSetting.rewardDetailList">
          <span v-if="index == 0">
...
//      appStorage.get('')
    ...
    methods: {
      getMarketSet() {
        api.getMarketSet().then((res) => {
          ...
          if(data.resultCode === '0') {
            ...
            this.followRewardSetting = Object.assign({},this.marketData.followRewardSetting);
          } else {
      ...
      marketFocusQrcode() {
        api.marketFocusQrcode().then(res => {
          const data = TOOL.toJson(res.data);
          if(data.resultCode === '0') {
            this.urlFirst = data.resultData.qrCodeUrl;
          } else {
            this.$store.dispatch('showError', data.resultMsg);
          }
        })
