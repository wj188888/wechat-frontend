<template>
  <div v-if="skus.length > 0">
    ...
    <div class="express-ticket-box line line-x-t" v-for="sku in skus"  @click="jumpToPage(sku.id, sku.price, sku.availableSeats)">
      <div class="order-box">
        <div class="start clearfix">
          <span class="time">{{ sku.startupTime | formatDate('time') }}</span>
    ...
    <!--<a class="button button-empty-white button-no-radius button-more line line-x-t">查看更多快车</a>-->
  </div>
</template>
<script>
