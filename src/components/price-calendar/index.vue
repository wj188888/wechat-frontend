      ...
      <tbody>
        <tr v-for="(day,k1) in days">
          ...
          @click="select(k1, k2, child, $event)">
            <span
            v-show="(!child.isLastMonth && !child.isNextMonth ) ||
            (child.isLastMonth && showLastMonth) ||
            (child.isNextMonth && showNextMonth)">
              {{replaceText(child.day, formatDate(year, month, child))}}
            </span>
            <p class="slot-box"
               v-show="(!child.isLastMonth && !child.isNextMonth ) ||
            (child.isLastMonth && showLastMonth) ||
            (child.isNextMonth && showNextMonth)" v-html="renderFunction(k1, k2, child)">
            </p>
          </td>
        </tr>
      </tbody>
